...
+(NSString *)ToJSONString_AuthorizationResult:(VKAuthorizationResult *)result
{
    ...
    @{
      ...
      @"error": [self ToJSONString_NSError:[result error]],
    };
...
+(NSString * _Nonnull)ToJSONString_VKError:(VKError * _Nullable)error
{
    if(!error)
    {
        return @"";
...
+(NSString * _Nonnull)ToJSONString_NSError:(NSError * _Nullable)error
{
    if(!error)
    {
        return @"";
